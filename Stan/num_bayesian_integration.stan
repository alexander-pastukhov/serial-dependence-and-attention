...
transformed data {
  int ParamsN = 2; // 1) A (power law constant), 2) S_sigma (uncertainty scaling)
...
parameters {
  vector[ParamsN * TaskN] mu_params; // 1) A (power law constant), 2) S_sigma (uncertainty scaling)
...
transformed parameters {
  ...
  vector[DataN] sigma;
  {
    ...
    matrix[TaskN, ParticipantsN] A = exp(block(params, 0 * TaskN + 1, 1, TaskN, ParticipantsN));
    matrix[TaskN, ParticipantsN] S_sigma = exp(block(params, 1 * TaskN + 1, 1, TaskN, ParticipantsN));
    ...
    for(i in 1:DataN) {
      if (IsFirstTrial[i]) {
        ...
        mu[i] = ((1 - W_response) * Numerosity[i] + W_response * Response[i-1]);
      }
...
model {
  ...
  mu_params[3:4] ~ normal(log(4), log(2));    // S_sigma
