...
transformed parameters {
  ...
  vector[DataN] sigma;
  {
    ...
    for(i in 1:DataN) {
      ...
      if (IsFirstTrial[i]) {
        mu[i] = Ori[i];
        total4average = Response[i];
        iAverageN = 1;
      } else {
        averageOri = total4average / iAverageN;
        ...
        mu[i] = Ori[i] + W_ct[Task[i], Participant[i]] * (averageOri - Ori[i]) + W_response * (Response[i-1] - Ori[i]) + W_ori * (Ori[i-1] - Ori[i]);
        total4average += Response[i];
        iAverageN += 1;
      }
    }
