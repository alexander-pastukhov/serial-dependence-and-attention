// Updated Bayesian Integration (ubi) model with Response weight (can take any value) and Evidence (orientation) weight (can take any value) and accounts for central tendency
data {
  int <lower=1> DataN;
  int <lower=1> TaskN;
...
transformed parameters {
  ...
  vector[DataN] OriCT;
  {
    ...
    matrix[TaskN, ParticipantsN] B_ct = block(params, 3 * TaskN + 1, 1, TaskN, ParticipantsN);

    for(i in 1:DataN) {
      sigma[i] = sigma_task[Task[i]];
      OriCT[i] = CentralOri + (Ori[i] - CentralOri) * B_ct[Task[i], Participant[i]];
      ...
      if (IsFirstTrial[i]) {
        ...
        mu[i] = OriCT[i] + W_response * (Response[i-1] - Ori[i]) + W_ori * (Ori[i-1] - Ori[i]);
      }
    }
  }
}
...
model {
 Response ~ normal(mu, sigma);
  ...
  mu_params[5:6] ~ normal(-0.5, 0.5);    // lambda
  mu_params[7:8] ~ normal(1, 0.5);     // B_ct
