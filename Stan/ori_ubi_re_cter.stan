// Updated Bayesian Integration (ubi) model with Response weight (can take any value) and Evidence (orientation) weight (can take any value) 
// and accounts for central tendency based on reponse
data {
  int <lower=1> DataN;
  int <lower=1> TaskN;
...
transformed data {
  int ParamsN = 5; // 1) W_response_max, 2) W_ori_max, 3) lambda (relevance), 4) B_ct
}
...
transformed parameters {
  ...
  vector[DataN] sigma;
  {
    ...
    matrix[TaskN, ParticipantsN] W_ct = block(params, 2 * TaskN + 1, 1, TaskN, ParticipantsN);
    matrix[TaskN, ParticipantsN] lambda = 2 * exp(block(params, 3 * TaskN + 1, 1, TaskN, ParticipantsN)).^2;
    matrix[TaskN, ParticipantsN] Wmix = inv_logit(block(params, 3 * TaskN + 1, 1, TaskN, ParticipantsN));

    real averageER = 0;
    real totalEvidence = 0;
    real totalResponse = 0;
    int iAverageN = 0;
    for(i in 1:DataN) {
      ...
      if (IsFirstTrial[i]) {
        ...
        averageER = (totalEvidence * Wmix[Task[i], Participant[i]] +  totalResponse * (1 - Wmix[Task[i], Participant[i]]))/ iAverageN;
        ...
        mu[i] = Ori[i] + W_ct[Task[i], Participant[i]] * (averageER - Ori[i]) + W_response * (Response[i-1] - Ori[i]) + W_ori * (Ori[i-1] - Ori[i]);
        totalEvidence += Ori[i];
        totalResponse += Response[i];
        iAverageN += 1;
      }
...
model {
 Response ~ normal(mu, sigma);
  ...
  mu_params[1:6] ~ normal(0, 0.5);     // w_response_max (1:2), w_ori_max (3:4) W_ct (5:6)
  mu_params[7:8] ~ normal(-0.5, 0.5);    // lambda
  mu_params[9:10] ~ normal(0, 0.5);     // w_response_max (1:2), w_ori_max (3:4) W_ct (5:6)
