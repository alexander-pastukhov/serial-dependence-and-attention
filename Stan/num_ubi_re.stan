...
model {
  mu_scale_params[1:2] ~ normal(logit(0.8), 1);  // scale; normal(logit(0.8), 0.5)
  mu_scale_params[3:4] ~ normal(0, 0.5);         // sigma_a; normal(0, 1)
  mu_scale_params[5:6] ~ normal(-2.5, 2);        // sigma_b; normal(0, 1)
  ...
  mu_params[3:4] ~ normal(-2.5, 2);  // sigma, so that kappa = 1/sigma; normal(-10, 1)
  mu_params[5:6] ~ normal(0, 1);     // W_response_max; normal(0.5, 0.5) 
  mu_params[7:8] ~ normal(0, 1);     // W_numerosity_max; normal(0.5, 0.5) 
  mu_params[9:10] ~ normal(-0.5, 2); // lambda; normal(0, 1)
