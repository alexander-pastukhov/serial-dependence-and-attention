...
transformed data {
  int ScaleParamsN = 4; // 1) B_ct (central tendency), 2) a_sigma, 3) b_sigma (linear model for sigma from uncertainty) 4) central numerosity
  int ParamsN = 5;
  ...
  // 4) W_numerosity_max (maximal weight for prior numerosity),
...
transformed parameters {
  ...
  vector[DataN] NumCT;
  {
    ...
    matrix[TaskN, ParticipantsN] B_ct = block(scale_params, 0 * TaskN + 1, 1, TaskN, ParticipantsN);
    ...
    matrix[TaskN, ParticipantsN] CentralNum = 100 * inv_logit(block(scale_params, 3 * TaskN + 1, 1, TaskN, ParticipantsN));
    ...
    matrix[TaskN, ParticipantsN] W_numerosity_max = block(params, 3 * TaskN + 1, 1, TaskN, ParticipantsN);
    matrix[TaskN, ParticipantsN] lambda = 2 * exp(block(params, 4 * TaskN + 1, 1, TaskN, ParticipantsN)).^2;
    ...
    for(i in 1:DataN) {
      ...
      NumCT[i] = CentralNum[Task[i], Participant[i]] + (Numerosity[i] - CentralNum[Task[i], Participant[i]]) * B_ct[Task[i], Participant[i]];
      ...
      if (IsFirstTrial[i]) {
        ...
        mu[i] = (NumCT[i] + W_response * (Response[i-1] - Numerosity[i]) + W_numerosity *  (Numerosity[i-1] - Numerosity[i]));
      }
