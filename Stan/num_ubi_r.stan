// Updated Bayesian Integration (ubi) model with Response weight (can take any value)
data {
  int <lower=1> DataN;
  int <lower=1> TaskN;
...
transformed data {
  ...
  // numerosity on a 0..1 scale for beta distribution
  ...
  vector[N] sigma;
  {
    ...
    for(i in 1:DataN) {
      ...
      if (IsFirstTrial[i]) {
        ...
        mu[i] = S_response[Task[i], Participant[i]] * (Numerosity[i] + W_response * (Response[i-1] - Numerosity[i]));
      }
...
model {
  ...
  mu_params[7:8] ~ normal(-0.5, 2); // lambda; normal(0, 1)
