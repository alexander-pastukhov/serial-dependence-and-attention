  ...
  vector[DataN] sigma;
  {
    ...
    for(i in 1:DataN) {
      if (IsFirstTrial[i]) {
        ...
        real Relevance_avg = exp(-((LogNumerosity[i] - avgEvidence)^2) / lambda[Task[i], Participant[i]]);
        real W_response = W_response_max[Task[i], Participant[i]] * Relevance_response;
        real W_numerosity = W_numerosity_max[Task[i], Participant[i]] * Relevance_numerosity;
