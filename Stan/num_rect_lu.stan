// Updated Bayesian Integration model with prior Response weight, prior Evidence (numerosity) weight and average Evidence weight (central tendency)
data {
  int <lower=1> DataN;
  int <lower=1> TaskN;
  ...
  vector[DataN] sigma;
  {
    ...
    for(i in 1:DataN) {
      ...
      if (IsFirstTrial[i]) {
        ...
        real Relevance_avg = exp(-((LogNumerosity[i] - log(avgEvidence))^2) / lambda[Task[i], Participant[i]]);
        real W_response = W_response_max[Task[i], Participant[i]] * Relevance_response * uncertainty;
        real W_numerosity = W_numerosity_max[Task[i], Participant[i]] * Relevance_numerosity * uncertainty;
