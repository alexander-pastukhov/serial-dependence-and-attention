...
results <-
  ...
  fs::dir_ls("Data/exp03-orientation", glob="*.csv") |>
...
for(a_model in model_names) {
  ...
  draws_name <- ("Draws/draws-exp3-{a_model}.RDS")
  ...
  saveRDS(draws_with_predictions_name, draws_name)
  
  draws_without_predictions <- draws |> select(starts_with("mu_params", "l_rho_params", "sigma_params", "sigma_params", "z_params", "sigma_task"))
  saveRDS(draws, draws_name)
}
```
