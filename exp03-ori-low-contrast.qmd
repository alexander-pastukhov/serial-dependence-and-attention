...
```{r}
extract_parameters <- function(draws_df, param_variable, param_names) {
  draws_df |>
  select(.draw, starts_with(param_variable)) |>
  pivot_longer(-.draw, names_to = "Term", values_to = "Estimate") |>
  mutate(iTerm = as.integer(str_extract(Term, "\\d")),
         Parameter = param_names[ceiling(iTerm / 2)],
         Task = factor((iTerm + 1) %% 2, labels = levels(results$Task))) |>
  select(.draw, Task, Parameter, Estimate)
}
params <- extract_parameters(draws, "mu_params", c("W_response_max", "W_ori_max", "lambda"))
```
...
plot_parameter_distribution(sigma_long, parameter_name = "sigma", long_df = TRUE)
