---
title: "Parameter Posterior Plots"
format: html
---
...
per task
```{r}
plot_triptych1 <- function(t, exp, model, width = 40, height = 13){
  plots <- list()
  for (a_exp in exp){
  ...
  plots
  ggsave(glue("Figures/{t}-{model}.png"), units = "cm", width = width, height = height)
  ggsave(glue("Figures/{t}-{model}.svg"), units = "cm", width = width, height = height)
}

experiments <- c("exp01", "exp02", "exp03")
plot_triptych1("abs", experiments, "rect_lu")
plot_triptych1("rel", experiments, "rect_lu")

#per task and order condition
plot_triptych1("abs", experiments, "rect_lu-order", width = 60, height = 13)
...
plot_triptych <- function(exp, parameter_name){
  ...
  for (a_exp in exp){
    ...
    if (parameter_name == "Prior Relevance"){
      plots[[a_exp]] <- readRDS(filename) + xlim(0,80)}
      else {plots[[a_exp]] <- readRDS(filename)}
  }
  purrr::reduce(plots, `+`) +
...
plot_triptych(experiments, "Log Prior Relevance")
ggsave("Figures/loglambda.png", units = "cm", width = 28, height = 7)
ggsave("Figures/loglambda.svg", units = "cm", width = 28, height = 7)


plot_triptych(experiments, "Sigma Max")
ggsave("Figures/sigmaMax.png", units = "cm", width = 28, height = 7)
ggsave("Figures/sigmaMax.svg", units = "cm", width = 28, height = 7)


# Sigma
((readRDS("ParameterPlots/posterior-distribution-exp01-Sigma.RDS"))|
...
posterior distributions for weights
```{r}
w01 <- readRDS("ParameterDraws/params01.RDS") |>
  filter(Parameter != "lambda")|>
  mutate(exp = "Exp.1: Numerosity")
w02 <- readRDS("ParameterDraws/params02.RDS") |>
  filter(Parameter != "lambda")|>
  mutate(exp = "Exp.2: Orientation (high contrast)")
w03 <- readRDS("ParameterDraws/params03.RDS") |>
  filter(Parameter != "lambda")|>
...
ggplot(weights, aes(x = Task, y = mu, color = Task, fill = Task)) +
  ...
  scale_y_continuous(name = "Weight", limits = c(-0.5, 1)) +
...
ggplot(aes(x = X, y = mu)) +
  ...
  facet_wrap(. ~ Parameter, labeller = labeller(Parameter = c("Wr" = "A) Prior Response", "We" = "B) Prior Evidence", "Wct" = "C) Average Evidence"))) +
