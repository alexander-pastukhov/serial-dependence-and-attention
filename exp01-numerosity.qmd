---
title: "Experiment 1: Numerosity"
format: html
---

## Libraries and settings required for analysis
```{r setup}
library(tidyverse)

library(cmdstanr)
library(fs)
library(glue)
library(kilter) # devtools:install_github("alexander-pastukhov/kilter", dependencies=TRUE)
library(loo)
library(patchwork)

source("utils.R")
```

```{r import data}
results <- 
  # get names for all csv files 
  fs::dir_ls("Data/exp01-numerosity/", glob="*.csv") |>

  # load them one at a time specifying the column structure
  purrr::map(~read_csv(., col_types = cols(
    Participant = col_character(),
    Session = col_character(),
    Block = col_integer(),
    Task = col_character(),
    Order = col_character(),
    Trial = col_integer(),
    OnsetDelay = col_double(),
    Ndots = col_integer(),
    Color = col_character(),
    ColorIsTarget = col_logical(),
    ColorResponse = col_character(),
    ColorRT = col_double(),
    NumerosityResponse = col_double(),
    NumerosityRT = col_double()
  ))) |>
  
  # bind into a single table
  list_rbind() |>
  
  rename(Numerosity = Ndots) |>
  
  # turning selected variables into factors
  mutate(Task = factor(Task, levels = c("single", "dual")),
         Order = factor(Order, levels = c("ascending", "random", "descending")),
         Participant = factor(Participant)) |>
  
  # computing whether response on central task was correct
  mutate(ColorTaskCorrect = (ColorIsTarget & ColorResponse == "target") | (!ColorIsTarget & ColorResponse ==  "distractor")) |>
  
  # convert numerosity response to number scale
  mutate(PN = NumerosityResponse,
         NumerosityResponse = NumerosityResponse * 100) |>
  
  # block 1: single central task
  filter(Block > 1) |>
  mutate(IsFirstTrial = Block == 2 & Trial == 1)
```


## Statistical analysis

Preparing data for Stan
```{r}
stan_data <- list(
  DataN = nrow(results),
  TaskN = nlevels(results$Task),
  ParticipantsN = nlevels(results$Participant),
  Numerosity_max = 100,

  Numerosity = results$Numerosity,
  Response = results$NumerosityResponse,
  Task = as.integer(results$Task),
  Participant = as.integer(results$Participant),
  IsFirstTrial = results$IsFirstTrial
)
```

Compiling models
```{r}
model_names <- c("log_linear", "bayesian_integration", "ubi_ur", "ubi_r", "ubi_re", "ubi_re_cte")#, "ubi_re_cte")
models <- map(model_names, ~cmdstan_model(glue("Stan/num_{.}.stan")))
names(models) <- model_names
```

Sampling
```{r}
for(a_model in model_names) {
  cat(a_model, "\n")
  loo_name <- glue("LOO/loo-exp1-{a_model}.RDS")
  draws_with_predictions_name <- glue("DrawsWithPredictions/draws-exp1-{a_model}.RDS")
  draws_name <- glue("Draws/draws-exp1-{a_model}.RDS")
  
  # if (fs::file_exists(loo_name) & fs::file_exists(draws_with_predictions_name) & fs::file_exists(draws_name)) next;
  if (fs::file_exists(loo_name) & fs::file_exists(draws_with_predictions_name)) next;
  
  fit <- models[[a_model]]$sample(stan_data, chains = 4, parallel_chains = future::availableCores())
  fit_loo <- fit$loo()
  saveRDS(fit_loo, loo_name) 
  
  draws <- fit$draws(format = "df") |> as_tibble() |> select(-starts_with("log_lik"))
  saveRDS(draws, draws_with_predictions_name)
  
  draws_without_predictions <- draws |> select(starts_with(c("mu_params", "l_rho_params", "sigma_params", "sigma_params", "z_params", "sigma_task")))
  saveRDS(draws_without_predictions, draws_name)
}
```


Comparing models via leave-one-out information criterion
```{r}
loos <- purrr::map(model_names, ~readRDS(glue("LOO/loo-exp1-{.}.RDS")))
names(loos) <- model_names
saveRDS(summarize_loo_comparison(loos), "LOO/loo-df-exp1.RDS")
summarize_loo_comparison(loos)
```

## Generating summary for predictions at the group level for absolute orientation

```{r}
draws <- readRDS("DrawsWithPredictions/draws-exp1-ubi_re_cte.RDS")
mu <- kilter::extract_stan_posterior_matrix(draws, "mu")
```

```{r}
# Bootstrapping behavioral averages
bootstrapped_abs <- bootstrap_group_averages("BootstrappedAverages/exp1-absolute-ubi_re_cte.RDS", results, Numerosity, NumerosityResponse, compute_average_absolute_response)

# Posterior predictions
posterior_abs <- posterior_group_averages_from_mu("PosteriorAverages/exp1-absolute-ubi_re_cte.RDS", results, mu, Numerosity, NumerosityResponse, compute_average_absolute_response)

plot <- plot_model_predictions("ubi_re_cte", posterior_abs, bootstrapped_abs, Numerosity, Response) +
  geom_abline(aes(intercept = 0, slope = 1), linetype = "longdash", linewidth = 0.7)
saveRDS(plot, file = glue("ParameterPlots/abs-posterior-prediction-exp01-ubi_re_cte.RDS"))

plot
ggsave("Figures/posterior-abs-exp01_ubi_re_cte.png", units = "cm", width = 16, height = 8)
ggsave("Figures/posterior-abs-exp01_ubi_re_cte.svg", units = "cm", width = 16, height = 8)
```

## Generating summary for predictions at the group level for relative orientation

```{r}
bootstrapped_rel <- bootstrap_group_averages("BootstrappedAverages/exp1-relative-ubi_re_cte.RDS", results, Numerosity, NumerosityResponse, compute_average_relative_response)

posterior_rel <- posterior_group_averages_from_mu("PosteriorAverages/exp1-relative-ubi_re_cte.RDS", results, mu, Numerosity, NumerosityResponse, compute_average_relative_response)

plot <- plot_model_predictions("ubi_re_cte", posterior_rel, bootstrapped_rel, Rel_Stimulus, Response) +
  geom_hline(yintercept = 0, linetype = "longdash", linewidth = 0.7) +
  geom_vline(xintercept = 0, linetype = "longdash", linewidth = 0.7)
saveRDS(plot, file = glue("ParameterPlots/rel-posterior-prediction-exp01-ubi_re_cte.RDS"))
plot
ggsave("Figures/posterior-rel-exp01_ubi_re_cte.png", units = "cm", width = 16, height = 10)
ggsave("Figures/posterior-rel-exp01_ubi_re_cte.svg", units = "cm", width = 16, height = 10)
```
## Plots for posterior distributions (for weights, prior evidence relevance and uncertainty)
```{r}
prior_relevance <- parameter_posterior_df(draws, "mu_params[7]", "mu_params[8]", "Prior_Relevance")|>
  mutate("Prior Relevance" = 2 * exp(Prior_Relevance)^2)|> 
  select(.draw, Task, "Prior Relevance")
plot_parameter_distribution(prior_relevance, "exp01", parameter_name = "Prior Relevance", long_df = TRUE)

plot_parameter_distribution(draws, "exp01", "Maximal Response Weight", "mu_params[1]", "mu_params[2]")
plot_parameter_distribution(draws, "exp01", "Maximal Evidence Weight", "mu_params[3]", "mu_params[4]")
plot_parameter_distribution(draws, "exp01", "Central Tendency Weight", "mu_params[5]", "mu_params[6]")

sigma_a <- parameter_posterior_df(draws, "mu_scale_params[5]", "mu_scale_params[6]", "Sigma_Min" )|>
  mutate("Sigma Min" = exp(Sigma_Min))
plot_parameter_distribution(sigma_a |> select(.draw, Task, "Sigma Min"), "exp01", parameter_name = "Sigma Min", long_df = TRUE)

sigma_max <- sigma_a|>
  mutate(sigma_b = parameter_posterior_df(draws, "mu_scale_params[7]", "mu_scale_params[8]", "sigma_b")[[3]],
         "Sigma Max" = exp(Sigma_Min) + exp(sigma_b))|>
  select(.draw, Task, "Sigma Max")
plot_parameter_distribution(sigma_max, "exp01", parameter_name = "Sigma Max", long_df = TRUE)
```


```{r}
p_seq <- seq(0.001, 0.999, length.out = 100)

u_params <- parameter_posterior_df(draws, "mu_scale_params[3]", "mu_scale_params[4]", "U_sigma" )|>
  mutate(U_max = inv_logit(parameter_posterior_df(draws, "mu_scale_params[1]", "mu_scale_params[2]", "U_max" )[[3]]),
         U_kappa = 1/exp(U_sigma))|>
  select (.draw, Task, U_max, U_kappa)|>
  mutate(a = U_max * U_kappa,
         b = (1 - U_max) * U_kappa,
         Unorm = dbeta(U_max, a, b))

uncertainty <- u_params |>
  group_by(.draw, Task) |>
  group_modify(~tibble(P = p_seq, U = dbeta(p_seq, .x$a[1], .x$b[1]) / .x$Unorm[1]))
  
uncertainty_avg <- 
  uncertainty |>
  group_by(Task, P) |>
  summarise(AvgU = mean(U),
            LowerCI = lower_ci(U),
            UpperCI = upper_ci(U),
            .groups = "drop")

ggplot(uncertainty_avg, aes(x = P * 100, y = AvgU, color = Task)) +
  geom_ribbon(aes(ymin = LowerCI, ymax = UpperCI, fill = Task), color = NA, alpha = 0.5) +
  geom_line() +
  scale_x_continuous(name = "Numerosity") +
  scale_y_continuous(name = "Uncertainty")+
  scale_fill_manual(values = c("single" = "#eb6972", "dual" = "#336a97")) +
  scale_color_manual(values = c("single" = "#eb6972", "dual" = "#336a97")) 

ggsave("Figures/uncertainty.png", units = "cm", width = 12, height = 8)
ggsave("Figures/uncertainty.svg", units = "cm", width = 12, height = 8)
```


## Plotting all weights together

```{r}
params01 <-
  draws |>
  select(.draw, starts_with("mu_params")) |>
  pivot_longer(starts_with("mu_params"), names_to = "Term", values_to = "mu") |>
  mutate(Index = as.integer(str_extract(Term, "\\d")),
         Task = factor((Index + 1) %% 2, labels = levels(results$Task)),
         Parameter = factor(floor((Index - 1) / 2), labels = c("Wr", "We", "Wct", "lambda"))) |>
  select(-Term, -Index)


params01 |>
  filter(Parameter != "lambda") |>
ggplot(aes(x = Task, y = mu, color = Task, fill = Task)) +
  #geom_boxplot(show.legend = FALSE, outlier.shape = NA, alpha = 0.5) +
  ggforce::geom_sina(show.legend = FALSE, alpha = 0.05) +
  scale_fill_manual(values = c("single" = "#e6444f", "dual" = "#00457d"))+
  scale_color_manual(values = c("single" = "#e6444f", "dual" = "#00457d"))+
  geom_hline(yintercept = 0, linetype = "dashed") +
  facet_wrap(. ~ Parameter) +
  scale_y_continuous(name = "Weight", limits = c(-0.25, 1))
```

