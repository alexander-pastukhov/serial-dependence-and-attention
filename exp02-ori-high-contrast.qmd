...
plot <- plot_model_predictions("ubi_re_ct", posterior_abs, bootstrapped_abs, Orientation, Response) +
...
ggsave("Figures/posterior-abs-exp02_ubi_re_ct.svg", units = "cm", width = 16, height = 10)
```
...
ggsave("Figures/posterior-rel-exp02_ubi_re_ct.svg", units = "cm", width = 16, height = 10)
```

## Plots for posterior distributions (for weights, prior evidence relevance and uncertainty)

```{r}
prior_relevance <- parameter_posterior_df(draws, "mu_params[5]", "mu_params[6]", "Prior_Relevance")|>
